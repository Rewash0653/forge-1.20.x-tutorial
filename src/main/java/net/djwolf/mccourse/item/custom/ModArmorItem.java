...
import net.djwolf.mccourse.item.ModArmorMaterials;
...
public class ModArmorItem extends ArmorItem {
    private static final List<MobEffect> EFFECTS = List.of(MobEffects.MOVEMENT_SPEED, MobEffects.WATER_BREATHING, MobEffects.DOLPHINS_GRACE);

    private static final Map<ArmorMaterial, MobEffectInstance> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
            .put(ModArmorMaterials.ALEXANDRITE, new MobEffectInstance(MobEffects.JUMP, 200, 1, false, false)).build();
    ...
    private void addEffectToPlayer(Player player, MobEffectInstance mapEffect) {
        for (MobEffect effect : EFFECTS) {
            if (!player.hasEffect(effect)) {
                player.addEffect(new MobEffectInstance(effect, mapEffect.getDuration(), mapEffect.getAmplifier(), mapEffect.isAmbient(), mapEffect.isVisible()));
            }
        }
    }
